...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300 relative flex flex-col">
      ...
      {/* ========================================================= */}
      {/* 🦶 FOOTER SECTION (Mandatory Requirement) */}
      {/* ========================================================= */}
      <footer className="bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 py-10 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-2 md:grid-cols-4 gap-8">
          <div className="col-span-2 md:col-span-1 space-y-2">
            ...
            <p className="text-xs text-slate-400 max-w-xs">Connecting curated culinary artists with conscious local communities. Your next meal matters.</p>
          </div>
          ...
          {([
            ...
            <div key={col.title}>
              <h6 className="font-bold text-xs text-slate-950 dark:text-white uppercase mb-2.5 tracking-wider">{col.title}</h6>
              <ul className="space-y-1.5">
                {col.links.map(link => (
                  <li key={link}>
                    <Link href="#" className="text-xs text-slate-500 dark:text-slate-400 hover:text-orange-600 transition-colors">{link}</Link>
                  </li>
                ))}
              </ul>
            </div>
